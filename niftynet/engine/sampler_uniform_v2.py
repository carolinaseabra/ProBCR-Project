...
# from niftynet.contrib.csv_reader.csv_reader \
#     import apply_niftynet_format_to_data
...
class UniformSampler(ImageWindowDatasetCSV):
    ...
    def layer_op(self, idx=None):
        ...
        # print(image_shapes, 'image_shapes are', self.window.names)
        ...
        for name in list(data):
            ...
            if len(image_array) > 1:
                ...
                # print(image_data_key, output_dict[image_data_key].shape)
            else:
                output_dict[image_data_key] = image_array[0]
                # print(output_dict[image_data_key].shape, 'is shape now')
        ...
        if self.csv_reader is not None:
            ...
            _, label_dict, s_id = self.csv_reader(idx=image_id)
            # print(image_id, label_dict['modality_label'],
            #       self.csv_reader.df_by_task['modality_label'].shape, s_id
            #       )
            # exit()
            ...
            for name in self.csv_reader.names:
                output_dict[name + '_location'] = output_dict[
                    'image_location']
                # print(output_dict[name].shape, output_dict[
                #     name+'_location'].shape, 'check_shapes_csv')
        ...
        # #################################################################
        # print("All prepared, sampling over")
        return output_dict
    ...
    def _spatial_coordinates_generator(self,
        ...
        for mod in list(win_sizes):
            # print(mod, win_sizes[mod][:N_SPATIAL], img_spatial_size)
            ...
            # print(spatial_coords[:,:N_SPATIAL], spatial_coords[:,N_SPATIAL:],
            #       img_spatial_size,
...
def rand_spatial_coordinates(
    ...
    for (idx, (img, win)) in enumerate(
        ...
        # print(max(img-win+1,1), img, win, 'check_sample')
