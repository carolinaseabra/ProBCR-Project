...
class ImageSetsPartitioner(object):
    ...
    def initialise(self,
        ...
        for key in data_param.keys():
            if not hasattr(data_param[key], 'csv_data_file'):
                self.data_param[key] = data_param[key]
            elif data_param[key].csv_data_file is None or len(data_param[\
                    key].csv_data_file) == 0:
                self.data_param[key] = data_param[key]
    ...
    def load_data_sections_by_subject(self, merge_multi=False):
        ...
        section_first = [section_name for section_name in
                         self.data_param if
                         not hasattr(self.data_param[
                                         section_name],
                                     'csv_data_file') or self.data_param[
                                         section_name].csv_data_file == '']
        ...
        for section_name in usable_section:
            ...
            n_rows = self._file_list.index.size
    ...
    def grep_files_by_data_section(self, modality_name):
        ...
        try:
            if 'csv_file' in mod_spec.keys():
                csv_path_file = os.path.expanduser(mod_spec.get('csv_file',
                                                            None))
            else:

                csv_path_file = os.path.expanduser(mod_spec.get('csv_path_file',
                                                            None))
            ...
            tf.logging.debug('`csv_path_file` not specified,'
                             ' writing the list of '
        ...
        ###############################
        try:
            if not (hasattr(self.data_param[modality_name], 'csv_path_file')
                or  hasattr(self.data_param[modality_name], 'csv_file')) or \
                (not hasattr(self.data_param[modality_name], 'csv_path_file')
                and self.data_param[modality_name].csv_file == '') or (
                    hasattr(self.data_param[modality_name], 'csv_path_file') and
                self.data_param[modality_name].csv_path_file == ''):
            ...
            elif (hasattr(self.data_param[modality_name], 'csv_path_file')
                    and self.data_param[modality_name].csv_path_file != '') \
                    or(hasattr(self.data_param[modality_name], 'csv_file')
                    and self.data_param[modality_name].csv_file != ''):
                csv_list = pandas.read_csv(
                    csv_path_file,
                    header=None,
                    dtype=(str, str),
        ...
        if temp_csv_path_file:
            shutil.rmtree(os.path.dirname(temp_csv_path_file),
                          ignore_errors=True)
