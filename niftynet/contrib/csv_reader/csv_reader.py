...
class CSVReader(Layer):
    ...
    def initialise(self, data_param, task_param=None, file_list=None,
        ...
        for name in valid_names:
            print('toohe is ', data_param[name].get('to_ohe', False))
    ...
    def _parse_csv(self, path_to_csv, to_ohe):
        ...
        if to_ohe and len(df.columns) == 1:
            ...
            _dims = 1
            # if (np.max(df[1].values) >1.1 ):

            _indexable_output = self.to_categorical(df[1].values,
                                                    np.sort(df[1].unique()))
            # _indexable_output = np.asarray(df[1].values, np.float32)
            ...
            _indexable_output = np.asarray(df.values, np.float32)
            # print(_indexable_output)
            # exit()
