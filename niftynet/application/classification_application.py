...
SUPPORTED_INPUT = set(['image', 'label', 'modality_label', 'sampler', 'inferred'])
...
class ClassificationApplication(BaseApplication):
    ...
    def initialise_dataset_loader(
        ...
        if self.is_training:
            samples_per_volume = self.action_param.sample_per_volume
        ...
        self.csv_readers = [
             CSVReader(csv_reader_names).initialise(
                 data_param, task_param, file_list, samples_per_volume) for file_list in
             file_lists]
    ...
    def initialise_uniform_sampler(self):
        self.sampler = [[UniformSampler(
            ...
            # queue_length = self.net_param.queue_length) for image_reader in
            # self.readers]]
    ...
    def connect_data_and_network(self,
        ...
        if self.is_training:
            ...
            # # Collect losses
            # outputs_collector.add_to_collection(
            #     var=class_loss, name='class_loss',
            #     average_over_devices=False, collection=CONSOLE)
            # outputs_collector.add_to_collection(
            #     var=class_loss, name='class_loss',
            #     average_over_devices=True, summary_type='scalar',
            ...
            # outputs_collector.add_to_collection(
            #     var=tf.contrib.image.rotate(255.0 * (image[:1, :, :, :] - tf.reduce_min(image[:1, :, :, :])) /
            #                                 (tf.reduce_max(
            #                                     image[:1, :, :, :] - tf.reduce_min(image[:1, :, :, :]))),
            #                                 3 * math.pi / 2), name='image',
