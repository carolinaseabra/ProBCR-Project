...
def main():
    ...
    if not options.template:
        ...
        options.template = fname + '-%d-%d' + 'png'
    ...
    img = prepare(filename, tile_size = [size, size], bgcolor = background, chatty = verbosity)

    subdivide(img, size = (size, size),
              quality = quality, filename = template, chatty = verbosity)


def prepare(filename, tile_size = [512, 512] ,bgcolor = background_default, chatty = chatty_default):
    """
    ...
    new_size[0] = math.ceil(src.size[0] / tile_size[0]) * tile_size[0]
    new_size[1] = math.ceil(src.size[1] / tile_size[1]) * tile_size[1]

    new_size_tp = tuple(new_size)
    img = Image.new('RGBA', new_size_tp, "#" + bgcolor)

    
    src.thumbnail(new_size_tp, scaling_filter)
    img.paste(src, (int((new_size_tp[0] - src.size[0]) / 2),
                    int((new_size_tp[1] - src.size[1]) / 2)))
    
    if chatty:
        print("full size: %s" % str(new_size_tp))
...
def tile(im, level, quadrant=(0, 0), size=(512, 512),
    ...
    if efficient:
        ...
        inverse_size = (float(im.size[0]) / float(size[0] * scale),
                        float(im.size[1]) / float(size[1] * scale))
        top_left = (int(quadrant[0] * size[0]),
                    int(quadrant[1] * size[1]))
        bottom_right = (int(top_left[0] + (size[0])),
                        int(top_left[1] + (size[1])))
        ...
        zoomed = im.crop(top_left + bottom_right).copy()
        return zoomed
...
def subdivide(img, level=0, quadrant=(0, 0), size=(512, 512),
    ...
    """
    counter_y = 0
    #for i in range(0, img.size[0]-1, size[0]):
    #    counter_y = 0
    #    for j in range(0, img.size[1]-1, size[1]):
    for i in range(0, int((img.size[0])/size[0])):
        counter_x = 0
        for j in range(0, int((img.size[1])/size[1])):
            print(i)
            print(j)
            quadrant = (i, j)
            out_img = tile(img, level=1, quadrant=quadrant, size=size)
            out_img.save((filename % (counter_x, counter_y)))
